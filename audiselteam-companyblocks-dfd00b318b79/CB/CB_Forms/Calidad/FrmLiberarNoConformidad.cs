...
namespace CB
{
    public partial class FrmLiberarNoConformidad : Ventana
    {
        public PlanoProyecto Plano = new PlanoProyecto();
        public NoConformidad NC = new NoConformidad();
        public string Disposicion = string.Empty;

        public FrmLiberarNoConformidad(int idPlano)
        {
            InitializeComponent();
            Plano.CargarDatos(idPlano);
            NC.SeleccionarPlano(idPlano);
        }
        ...
        {
            foreach (Fila f in NC.Filas())
            {
                f.ModificarCelda("disposicion", Disposicion);
                f.ModificarCelda("estatus", "LIBERADO");
                f.ModificarCelda("usuario_liberacion", Global.UsuarioActual.Fila().Celda("id"));
