...
namespace CB
{
    public partial class FrmMonitorCalidad2 : Form
    {
        ...
        private void CargarHerramentista()
        {
            ...
            Usuario.Modelo().SeleccionarRolActivos("TECNICO HERRAMENTISTA").ForEach(delegate(Fila f)
            {
                CmbHerramentista.Items.Add(f.Celda("nombre") + " " + f.Celda("paterno"));
            });

            CmbHerramentista.Text = "TODOS";
        }
        ...
        private void tabControl1_SelectedIndexChanged(object sender, EventArgs e)
        {
            switch (tabControl1.SelectedIndex)
            {
                case 3: // NC Externas
                    ...
                    CargarHerramentista();
                    CargarProyectosDefectos();
                    CmbEstatusDefecto.Text = "SIN LIBERAR";
                    CargarNcExternas(DtpDesdeDefecto.Value, DtpHastaDefecto.Value);
                    break;
                case 4: // NC Internas
        ...
        private void CargarProyectosDefectos()
        {
            CmbProyectoDefecto.Items.Clear();
            CmbProyectoDefecto.Items.Add("TODOS");
            DefectoFabricacion.Modelo().SeleccionarProyectosConDefectos().ForEach(delegate(Fila f)
            {
                CmbProyectoDefecto.Items.Add(Convert.ToDecimal(f.Celda("proyecto")).ToString("F2") + " - " + f.Celda("nombre"));
            });
            CmbProyectoDefecto.Text = "TODOS";
        }
        ...
        public void CargarNcExternas(DateTime desde, DateTime hasta)
        {
            NoConformidad nc = new NoConformidad();
            DefectoFabricacion defectos = new DefectoFabricacion();

            string estatus = string.Empty;

            if (CmbEstatusDefecto.Text == "SIN LIBERAR")
                estatus = "DEFECTO";
           ...
           // nc.SeleccionarTipo(CmbTipoNcExterna.Text, CmbEstatusAnalisisNcExterna.Text).ForEach(delegate(Fila f)
            defectos.SeleccionarDefectos(desde, hasta, CmbHerramentista.Text, estatus, CmbProyectoDefecto.Text).ForEach(delegate(Fila f)
            {
                string strDetalles = string.Empty;

                strDetalles += Convert.ToDecimal(f.Celda("proyecto")).ToString("F2").PadLeft(6, '0') + " - " + f.Celda("nombre_proyecto") + Environment.NewLine;
                strDetalles += "PLANO: " + f.Celda("id_proyecto") + " " + f.Celda("nombre_archivo") + Environment.NewLine;

                DefectoFabricacion.Modelo().SeleccionarDefectosPorPlanos(Convert.ToInt32(f.Celda("id_proyecto"))).ForEach(delegate(Fila filaProceso)
                {
                    strDetalles +=  Environment.NewLine;
                    strDetalles += "PROCESO: " + filaProceso.Celda("proceso") + Environment.NewLine;
                    strDetalles += "DEFECTO: " + filaProceso.Celda("tipo_defecto") + Environment.NewLine;
                    strDetalles += "DEFECTOS ENCONTRADOS: " + filaProceso.Celda("cantidad_nok") + Environment.NewLine;
                    strDetalles += "DETALLES: " + filaProceso.Celda("detalles_defecto") + Environment.NewLine;
                    strDetalles += "HERRAMENTISTAS: " + filaProceso.Celda("operador") + Environment.NewLine;
                });
                

                DgvNcExternas.Rows.Add(f.Celda("id").ToString().PadLeft(4, '0'), strDetalles, f.Celda("estatus"), f.Celda("id_proyecto"));
            });
        }
        ...
        public int CrearRequisicionMaterial(int idCatalogo, int cantidad)
        {
            ...
            {
                MessageBox.Show("No se puede seleccionar, el material ha sido restringido por un líder de proyecto, favor consultarlo", "Material restringido", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return 0;
            }
        ...
        public void InspeccionarProcesoFabricacion()
        {
            ...
            if (inspeccionar.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
                ...
                if (inspeccionar.RechazarPlano) // hubo un rechazo
                {
                    ...
                    DialogResult resp = MessageBox.Show("Quieres liberar ahora los productos con defecto?", "Liberar defecto", MessageBoxButtons.YesNo, MessageBoxIcon.Question);

                    if (resp == System.Windows.Forms.DialogResult.Yes)
                    {
                        FrmLiberarDefecto liberar = new FrmLiberarDefecto(inspeccionar.IdPlano);
                        ...
                        if (liberar.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                        {
                            ...
                            {
                                case "SCRAP":
                                    MessageBox.Show("Coloca el producto con defecto en contenedor de scrap", "Producto liberado como scrap", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                    break;

                                case "REPARACION":
                                    MessageBox.Show("Entrega el producto con defecto al Supervisor de tool room para que sea reparado. Asegúrate de que tenga la etiqueta roja con el ID de trazabilidad.", "Producto liberado para reparación", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                    break;
                            ...
                            MessageBox.Show("Coloca el producto con defecto en el rack de producto con defecto", "Producto con defecto", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                    else
                        MessageBox.Show("Coloca la pieza en el rack de producto con defecto", "Pieza rechazada", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                else
                {
                    MessageBox.Show("Selecciona el siguiente proceso", "Proceso siguiente", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        {
            FrmResultadosInspeccion2 noConformidad = new FrmResultadosInspeccion2(Plano, IdProceso);
            noConformidad.Show();
        ...
        private void liberarToolStripMenuItem_Click(object sender, EventArgs e)
        {
           if (DgvNcInternas.SelectedRows.Count == 0) return;
            ...
            FrmLiberarDefecto nc = new FrmLiberarDefecto(idPlano);
            ...
            {
                MessageBox.Show("Coloca el producto con defecto en el rack de no conformidades.", "Liberar no conformidad interna", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        {
            if(DtpDesdeDefecto.Value.Date <= DtpHastaDefecto.Value.Date)
            {
                CargarNcExternas(DtpDesdeDefecto.Value, DtpHastaDefecto.Value);
            }
        }
        ...
        private void comboBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            DateTime esteMes = DateTime.Now;

            switch (CmbFecha.Text)
            {
                case "ESTA SEMANA":
                    desde = Global.InicioSemana(DateTime.Now);
                    hasta = desde.AddDays(4);
                    break;
                case "LA SEMANA PASADA":
                    desde = Global.InicioSemanaPasada();
                    hasta = desde.AddDays(4);
                    break;
                case "ESTE MES":
                    desde = Global.FechaInicialMes(DateTime.Now);
                    hasta = desde.AddDays(DateTime.DaysInMonth(DateTime.Now.Year, DateTime.Now.Month) -1);
                    break;
                case "EL MES PASADO":
                    esteMes = new DateTime(DateTime.Now.Year, DateTime.Now.Month, 1);
                    desde = Global.FechaInicialMes(esteMes.AddMonths(-1));
                    hasta = desde.AddDays(DateTime.DaysInMonth(DateTime.Now.Year, DateTime.Now.Month) -1);
                    break;
                case "ESTE BIMESTRE":
                    esteMes = new DateTime(DateTime.Now.Year, DateTime.Now.Month, 1);
                ...
                case "ESTE SEMESTRE":
                    break;
                case "ESTE AÑO":
                    desde = Convert.ToDateTime(DateTime.Now.Year + "/01/01");
                    hasta = Convert.ToDateTime(DateTime.Now.Year + "/12/31");
                    break;
                case "EL AÑO PASADO":
                    desde = Convert.ToDateTime((DateTime.Now.Year -1) + "/01/01");
                    hasta = Convert.ToDateTime((DateTime.Now.Year - 1) + "/12/31");
                    break;
                default:
                    desde = DateTime.Now;
                    hasta = DateTime.Now;
            ...
            DtpDesdeDefecto.Value = desde;
            DtpHastaDefecto.Value = hasta;
        }
        ...
        private void DtpDesdeDefecto_ValueChanged(object sender, EventArgs e)
        {
            if (DtpDesdeDefecto.Value.Date <= DtpHastaDefecto.Value.Date)
            {
                CargarNcExternas(DtpDesdeDefecto.Value, DtpHastaDefecto.Value);
        ...
        private void MenuDefecto_Opening(object sender, CancelEventArgs e)
        {
            toolStripMenuItem1.Visible = (DgvNcExternas.SelectedRows.Count == 1 && DgvNcExternas.SelectedRows[0].Cells["estatus_defecto"].Value.ToString() == "DEFECTO");
        }

        private void CmbEstatusDefecto_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (DtpDesdeDefecto.Value.Date <= DtpHastaDefecto.Value.Date)
            {
                CargarNcExternas(DtpDesdeDefecto.Value, DtpHastaDefecto.Value);
        ...
        {
            if (DgvNcExternas.SelectedRows.Count == 0) return;

            int idPlano = Convert.ToInt32(DgvNcExternas.SelectedRows[0].Cells["id_plano_defecto"].Value);
        ...
        private void CmbProyectoDefecto_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (DtpDesdeDefecto.Value.Date <= DtpHastaDefecto.Value.Date)
            {
                CargarNcExternas(DtpDesdeDefecto.Value, DtpHastaDefecto.Value);
