...
namespace CB
{
    public partial class FrmResultadosInspeccion2 : Ventana
    {
        int IdPlano = 0;
        int IdProceso = 0;
        byte[] PlanoImagen = null;

        public FrmResultadosInspeccion2(int idPlano, int idpProceso)
        {
            InitializeComponent();
            IdPlano = idPlano;
        ...
        private void FrmConsultarNoConformancia_Load(object sender, EventArgs e)
        {
            ...
            DefectoFabricacion defecto = new DefectoFabricacion();
            ...
            if (proyecto.TieneFilas())
            {
                ...
                int count = 0;
                List<Fila> ListaNoConformidades = defecto.SeleccionarDefectosDePlano(IdPlano);// noConformidad.SeleccionarNoConformidad(IdPlano, IdProceso);
                if(ListaNoConformidades.Count > 0)
                {
                    ListaNoConformidades.ForEach(delegate(Fila f)
                    {
                        ...
                        TxtComentarios.Text += "(" + count + ").-PROCESO RECHAZADO: " + f.Celda("proceso_fabricacion_rechazado").ToString() + " " + f.Celda("proceso").ToString();
                        ...
                        TxtComentarios.Text += Environment.NewLine + "REVISADO POR: " + f.Celda("usuario_creacion").ToString();
